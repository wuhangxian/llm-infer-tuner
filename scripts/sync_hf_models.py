...
def extract_mtp_params_from_cookbook(mdx_path: Path) -> dict[str, Any]:
    ...
    if m:
        params["speculative-algorithm"] = m.group(1).upper()
...
def generate_new_model_card(hf_id: str, info: dict, sglang_repo: str = "") -> dict[str, Any]:
    ...
    # mtp_params from cookbook (kept for compatibility). The structured
    # speculative_options field is the extensible form consumed by the skill.
    if cookbook_mtp:
            ...
            for mk, mv in models.items():
                if mv.get("hf_model_id") == hf_id:
                    ...
                    if mdx:
                        ...
                        # Always inspect MTP metadata. This repairs old cards
                        # polluted by cookbook punctuation and records the
                        # algorithm without hard-coding EAGLE in the generator.
